...
Iceberg integrates with metadata catalogs such as **AWS Glue**, **Hive Metastore**, **Project Nessie**, and **Apache Polaris**, allowing unified table discovery, access control, and schema management.
Combined with features like **Z-order clustering**, **automatic file compaction**, and **metadata pruning**, Iceberg delivers warehouse-grade performance on top of open storage.
...
For example, Flink’s Iceberg connector enables exactly-once ingestion:
...
Iceberg’s **automatic file compaction** consolidates the small files generated by streaming writes, preventing performance degradation and maintaining healthy table structures over time.
