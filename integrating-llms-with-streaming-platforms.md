...
For detailed coverage of RAG with streaming data, see [RAG Pipelines with Real-Time Data](rag-pipelines-with-real-time-data.md).
...
For real-time embedding strategies, see [Vector Embeddings in Streaming](vector-embeddings-in-streaming.md). For architectural patterns connecting vector databases with streaming platforms, see [Vector Databases and Streaming Architectures](vector-databases-and-streaming-architectures.md).
...
For CDC patterns that enable these feedback loops, see [What is Change Data Capture? CDC Fundamentals](what-is-change-data-capture-cdc-fundamentals.md).
...
This pattern creates feedback loops where production data continuously improves model performance, though it requires careful attention to data quality and privacy. For ETL patterns, see [Streaming ETL vs Traditional ETL](streaming-etl-vs-traditional-etl.md).
...
For Kafka Connect fundamentals, see [Kafka Connect: Building Data Integration Pipelines](kafka-connect-building-data-integration-pipelines.md). For transformation patterns, see [Kafka Connect Single Message Transforms](kafka-connect-single-message-transforms.md).
...
For foundational Kafka Streams concepts, see [Introduction to Kafka Streams](introduction-to-kafka-streams.md).
...
For managing state in stream processing, see [State Stores in Kafka Streams](state-stores-in-kafka-streams.md). For autonomous agents built on streaming data, see [Agentic AI Pipelines: Streaming Data for Autonomous Agents](agentic-ai-pipelines.md).
...
For foundational Kafka concepts, see [Apache Kafka](apache-kafka.md). For event-driven architecture principles, see [Event-Driven Architecture](event-driven-architecture.md). For ML integration patterns, see [Real-Time ML Inference with Streaming Data](real-time-ml-inference-with-streaming-data.md) and [Real-Time ML Pipelines](real-time-ml-pipelines.md).
